...
def get_key():
    pub_key = open('jwtRS256.key.pub').read()

    if pub_key is not None:
        return good_json_response(open('jwtRS256.key.pub').read())
    else:
        return bad_json_response("Error retrieving the public key of server: " + get_own_ip())
